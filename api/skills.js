...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { data, error } = await supabase.from('skills').insert(clean(req.body)).select().single();
      if (error) throw error;
    ...
    if (req.method === 'PUT') {
      ...
      const { data, error } = await supabase.from('skills').update(clean(patch)).eq('id', rowId).select().single();
      if (error) throw error;
      return res.status(200).json(data);
    }
    if (req.method === 'DELETE') {
      ...
      const rowId = validId(id);
      if (!rowId) return res.status(400).json({ error: 'Invalid input.' });
      const { error } = await supabase.from('skills').delete().eq('id', rowId);
      if (error) throw error;
      return res.status(200).json({ ok: true });
    }
    return res.status(405).json({ error: 'Method not allowed' });
  } catch (err) {
    return serverError(res, err, 'skills api error');
  }
}
